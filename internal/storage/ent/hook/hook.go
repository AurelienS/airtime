...
// The FlightStatisticFunc type is an adapter to allow the use of ordinary
// function as FlightStatistic mutator.
type FlightStatisticFunc func(context.Context, *ent.FlightStatisticMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f FlightStatisticFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.FlightStatisticMutation); ok {
		return f(ctx, mv)
	}

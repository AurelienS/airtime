...
func (sq *SquadQuery) QueryMembers() *UserQuery {
...
func (sq *SquadQuery) WithMembers(opts ...func(*UserQuery)) *SquadQuery {
	query := (&UserClient{config: sq.config}).Query()
...
func (sq *SquadQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Squad, error) {
	...
	if query := sq.withMembers; query != nil {
		if err := sq.loadMembers(ctx, query, nodes,
			func(n *Squad) { n.Edges.Members = []*User{} },
			func(n *Squad, e *User) { n.Edges.Members = append(n.Edges.Members, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SquadQuery) loadMembers(ctx context.Context, query *UserQuery, nodes []*Squad, init func(*Squad), assign func(*Squad, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Squad)
	nids := make(map[int]map[*Squad]struct{})
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(squad.MembersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(squad.MembersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(squad.MembersPrimaryKey[1]))

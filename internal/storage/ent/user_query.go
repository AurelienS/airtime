...
func (uq *UserQuery) QueryFlights() *FlightQuery {
	query := (&FlightClient{config: uq.config}).Query()
...
func (uq *UserQuery) WithFlights(opts ...func(*FlightQuery)) *UserQuery {
	query := (&FlightClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withFlights; query != nil {
...
func (uq *UserQuery) loadFlights(ctx context.Context, query *FlightQuery, nodes []*User, init func(*User), assign func(*User, *Flight)) error {
	...
	query.Where(predicate.Flight(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.FlightsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_flights
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_flights" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {

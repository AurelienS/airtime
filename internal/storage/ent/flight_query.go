...
func (fq *FlightQuery) QueryPilot() *UserQuery {
...
func (fq *FlightQuery) WithPilot(opts ...func(*UserQuery)) *FlightQuery {
	query := (&UserClient{config: fq.config}).Query()
...
func (fq *FlightQuery) loadPilot(ctx context.Context, query *UserQuery, nodes []*Flight, init func(*Flight), assign func(*Flight, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Flight)
	for i := range nodes {
		if nodes[i].user_flights == nil {
			continue
		}
		fk := *nodes[i].user_flights
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_flights" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}

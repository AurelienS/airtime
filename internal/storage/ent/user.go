...
type User struct {
	...
	// GoogleID holds the value of the "googleID" field.
...
func (e UserEdges) FlightsOrErr() ([]*Flight, error) {
	if e.loadedTypes[0] {
	...
	for i := range columns {
		...
		case user.FieldGoogleID:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field googleID", values[i])
			} else if value.Valid {
